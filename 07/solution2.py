...
class Hand:
    def __init__(self, data: str):
        ...
        self.strength: int = self._determine_strength(self.cards)
    ...
    def _determine_strength(self, cards: list[Card]) -> int:
        ...
        if "J" in [x.value for x in cards]:
            possible_strengths = []
            # Keys[:-1] so we don't replace jack with jack
            for new_card in list(CARD_RANK.keys())[1:]:
                new_cards_str = "".join([x.value for x in cards]).replace(
                    "J", new_card, 1
                )
                new_cards = [Card(x) for x in new_cards_str]
                possible_strengths.append(self._determine_strength(new_cards))
...
def main(args: Optional[Sequence[str]] = None) -> None:
    ...
    for x in [f"{i} -> {x} -> {i * x.bid}" for i, x in enumerate(sorted(hands), 1)]:
        print(x)
    print(winnings)
