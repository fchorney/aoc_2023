...
class Coordinate:
    ...
    def move(self, network: "Network", direction: Direction) -> Self | None:
        test_dir = None
        check = None
        x = self.x
        y = self.y
        ...
        if check or network.data[y][x] not in test_dir:
            return None
...
class Network:
    ...
    def find_furthest_location(self) -> int:
        if not self.start_location:
            return -1
...
def main(args: Optional[Sequence[str]] = None) -> None:
    ...
    furthest_idx = network.find_furthest_location()
    ic(furthest_idx)
    print(furthest_idx / 2)
